...
function buildUiModal({ message, kind, defaultValue = '', okText = 'موافق', cancelText = 'إلغاء', masked = false }) {
  return new Promise(resolve => {
    ...
    const box = createElement('div', { class: 'ui-modal' }, `
      ...
      ${kind === 'prompt'
        ? `<input type="${masked ? 'password' : 'text'}" class="ui-modal-input" id="uiModalInput"
...
/* كلمة المرور تُخفى أثناء الكتابة.
   كانت تُكتب في حقل `type="text"` عادي، أي مقروءة لمن يقف خلف الشاشة —
   واللعبة تُلعب في مجالس وأمام الناس، فهذا ليس افتراضاً نظرياً. و`password`
   يجعل مدير كلمات المرور يعرض الحفظ والملء تلقائياً بدل تجاهل الحقل. */
function uiPromptPassword(message) {
  return buildUiModal({ message, kind: 'prompt', okText: 'تم', masked: true });
}
